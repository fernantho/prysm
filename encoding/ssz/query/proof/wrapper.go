...
func (w *Wrapper) CommitWithMixin(i, num, limit int) {
	// Convert limit to next power of 2 (fastssz CalculateLimit returns the number of chunks,
	// which may not be a power of 2, but our tree implementation requires power of 2)
	powerOfTwoLimit := int(nextPowerOfTwo(uint64(limit)))

	// create tree from nodes
	res, err := TreeFromNodesWithMixin(w.nodes[i:], num, powerOfTwoLimit)
	if err != nil {
		panic(err)
	}
	// remove the old nodes
